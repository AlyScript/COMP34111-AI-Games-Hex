...
class AmafNode:
    def __init__(self, parent=None, move=None):
        ...
        self.wins = 0  # Not used for selection in Pure AMAF
        self.visits = 0
        self.amaf_wins = 0
        self.amaf_visits = 0


class AmafAgent(AgentBase):
    ...
    def make_move(self, turn: int, board: Board, opp_move: Move | None) -> Move:
        ...
        # MCTS Loop
        root = AmafNode(parent=None, move=None)
        ...
        while time.time() - start_time < time_budget:
            ...
            # 1. SELECTION (Pure AMAF)
            while node.untried_moves == [] and node.children:
                node = self.select_child_amaf(node)
                depth = 0
                t = node
                while t.parent:
                    depth += 1
            ...
            while node:
                node.visits += 1
                # Standard wins still tracked for final selection, but not tree selection
                node.wins += win_val
        ...
        # Robust Child (best actual visits) is still the safest final choice
        best = max(root.children, key=lambda c: c.visits)
        r, c = divmod(best.move, self.board_size)
        return Move(r, c)

    def select_child_amaf(self, node):
        """
        Pure AMAF Selection.
        We completely ignore node.wins and node.visits (local stats).
        We only look at node.amaf_wins and node.amaf_visits (global stats).
        """
        best_score = -float("inf")
        best_child = None

        for child in node.children:
            # If a move has never been seen in ANY simulation passing through here, try it
            if child.amaf_visits == 0:
                return child

            # Pure AMAF Score
            score = child.amaf_wins / child.amaf_visits

            # Havent added exploration (UCT) here because AMAF values should converge very quickly.
